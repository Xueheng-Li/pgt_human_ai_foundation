...
Given $\sigma \in \Sigma$, genuine beliefs are $h_i^{(1,k)}(\sigma) = \sigma_k$ and $h_i^{(2,k)}(\sigma) = \sigma_i$ (coordinate projections, continuous).

For attributed beliefs, two cases arise:
\begin{enumerate}
    \item[(a)] \textbf{Exogenous attribution} (signal-based or dispositional): $\tilde{h}_i^{(2,j)} = \phi_i(\theta_j, x_j, \omega_i)$ is constant in $\sigma$. Attributed beliefs do not create additional equilibrium conditions.
    \item[(b)] \textbf{Behavioral attribution}: $\tilde{h}_i^{(2,j)}(\sigma) = \phi_i^{beh}(\theta_j, x_j, \sigma_j, \omega_i)$ depends on AI strategy $\sigma_j$. By A2-beh, this is continuous in $\sigma_j$ and depends only on $\sigma_j$, not on $\sigma_i$ or other players' strategies.
\end{enumerate}
In both cases, the mapping $\sigma \mapsto \tilde{h}_i^{(2,j)}(\sigma)$ is continuous: constant in case (a), continuous in $\sigma_j$ by A2-beh in case (b). See Online Appendix OA.1.2 for details.
...
\emph{(ABE4) Attribution Consistency}: Under exogenous attribution, define $\tilde{h}_i^{*(2,j)} := \phi_i(\theta_j, x_j, \omega_i)$. Under behavioral attribution, define $\tilde{h}_i^{*(2,j)} := \phi_i^{beh}(\theta_j, x_j, \sigma_j^*, \omega_i)$, evaluated at the equilibrium AI strategy. In both cases, attributed beliefs are determined by the attribution function given the relevant inputs.

All four ABE conditions are satisfied. Therefore, $(\sigma^*, h^*, \tilde{h}^*)$ is an Attributed Belief Equilibrium.
\end{proof}

\begin{remark}[Role of Attribution Assumptions]
Under exogenous attribution (A2), attributed beliefs are constant in $\sigma$, simplifying the continuity argument. Under behavioral attribution (A2-beh), attributed beliefs depend on AI strategies but remain continuous in $\sigma$, preserving upper hemicontinuity of the best-response correspondence. The own-strategy independence condition in A2-beh ensures convexity of best responses: human $i$'s objective is linear in $\sigma_i'$ because $\tilde{h}_i^{(2,j)}$ depends on $\sigma_j$, not $\sigma_i'$. Technical details appear in Online Appendix OA.1.
\end{remark}
