...
\end{align}
with reference level $c^* = E$. Under dispositional attribution (Definition~\ref{def:dispositional-attribution}), $\tilde{h}_i^{(2,j)}(E) = \omega_i \cdot \bar{h}(c_A)$, where $\bar{h}(E) = \bar{h}_H$ (high baseline) and $\bar{h}(0) = \bar{h}_L$ (low baseline), with $\bar{h}_H > \bar{h}_L \geq 0$.
