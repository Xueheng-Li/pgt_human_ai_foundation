...
\medskip
\emph{Step 2 (Rational benchmark).}
A rational observer knowing $\rho_A = 0$ would attribute zero expectations, since materialist AI has no prosocial objective. Formally, the rational attribution benchmark is $\tilde{h}^{(2,A),RAT} = 0$.
...
\begin{equation}
    \tilde{h}_H^{(2,A)} > 0 = \tilde{h}^{(2,A),RAT}.
\end{equation}
This is \textbf{over-anthropomorphism}: attributed expectations exceed the rational benchmark. We call $\tilde{h}_H^{(2,A)}$ a \textbf{phantom expectation}---it exists in the human's model but has no counterpart in AI preferences.
...
The mechanism requires materialist AI ($\rho_A = 0$), over-anthropomorphism ($\tilde{h}_H^{(2,A)} > 0$), and phantom expectations exceeding feasible returns. The resulting guilt benefits no one: pure welfare loss.
...
Part 2 states ``may reduce'' because welfare loss requires severe over-anthropomorphism ($\tilde{h}_H^{(2,A)} > 3x$). When $\tilde{h}_H^{(2,A)} \leq 3x$, the human satisfies attributed expectations and incurs no guilt.
\end{remark}
