% Proof of Proposition: Rational Attribution and Bayesian Game Equivalence
...
Given the psychological game $\Gamma$ and a candidate equilibrium $\sigma^*$ satisfying rational attribution, construct the Bayesian game $\Gamma^B$ as follows:
...
\begin{itemize}
    ...
    \end{equation}
    Under rational attribution, all second-order beliefs (genuine and attributed) equal $\sigma_i^*$. The psychological payoff $\psi_i$ is evaluated at these consistent beliefs.
...
\medskip
\noindent\textbf{Part (a): ABE with Rational Attribution $\Rightarrow$ BNE.}

Let $(\sigma^*, h^*, \tilde{h}^*)$ be an ABE of $\Gamma$ where $(\sigma^*, \phi)$ satisfy rational attribution. We show that $\sigma^*$ is a BNE of $\Gamma^B$.

\emph{Step 1: Characterize equilibrium beliefs under rational attribution.}
...
Therefore, under rational attribution, all second-order beliefs---both genuine ($h_i^{*(2,k)}$) and attributed ($\tilde{h}_i^{*(2,j)}$)---equal the equilibrium strategy $\sigma_i^*$.
...
\medskip
\noindent\textbf{Part (b): BNE $\Rightarrow$ ABE with Rational Attribution.}
...
\textbf{(ABE1) Human Optimality.} Since $\sigma^*$ is a BNE of $\Gamma^B$, and the payoffs coincide under rational attribution (Step 2 of Part (a)), human optimality in ABE is satisfied.
...
The mappings in Parts (a) and (b) are consistent: the same strategy profile $\sigma^*$ appears in both the ABE and the BNE when rational attribution holds. This establishes the correspondence.
\end{proof}
