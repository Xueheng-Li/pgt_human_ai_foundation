...
\begin{remark}[Attribution Type in Applications]
\label{rem:exogenous-applications}
All applications in this section use \textbf{exogenous attribution}---signal-based or dispositional---where attributed beliefs depend on AI design parameters, interface features, and human anthropomorphism tendency, not on equilibrium strategies. This ensures attributed beliefs are determined before the game is played, satisfying Assumption A2. For extensions involving behavioral attribution (where attributed beliefs depend on observed AI behavior), see Assumption A2-beh and the discussion in Online Appendix OA.1.3.
\end{remark}
...
\begin{remark}[Design Choice: AI Trustor]
We position AI as trustor and human as trustee, inverting the natural test of whether humans trust AI. This design isolates the attribution mechanism: with AI as sender, the human's return decision reveals whether attributed expectations (about what AI ``expected'') trigger guilt-driven reciprocity. Absent anthropomorphism ($\omega_H \approx 0$), material incentives favor returning zero. Positive anthropomorphism elevates attributed expectations, increasing guilt from low returns. The design thus tests whether anthropomorphic cues affect human behavior through the psychological channel specified in Definition~\ref{def:guilt}.
\end{remark}
...
\begin{remark}[Notation: Transfer vs. Signal]
\label{rem:transfer-signal}
The trust game involves two distinct variables denoted by ``$x$'':
\begin{itemize}
    \item \textbf{Monetary transfer} $x \in [0, E]$: the strategic choice of how much the AI trustor sends.
    \item \textbf{Interface signal} $x_A$: anthropomorphic design features (interface style, voice, emotional expression) that enter the attribution function.
\end{itemize}
The attributed belief $\tilde{h}_H^{(2,A)} = \phi_H(\rho_A, x_A, \omega_H)$ depends on the interface signal $x_A$, \textbf{not} the monetary transfer $x$. This is exogenous attribution: the human's attributed expectations about ``what the AI expected'' are determined by AI design parameters and interface features, not by the AI's equilibrium strategy. Section~\ref{sec:empirical-implementation} varies $x_A \in \{x_A^R, x_A^H, x_A^C\}$ (robotic, humanoid, control) to test how interface signals affect attributed beliefs.
\end{remark}
...
Expectation conformity extends guilt and indignation motives to coordination: humans experience pressure to match perceived social expectations \citep{cialdini_Social_1998a}. Through attributed beliefs, this mechanism operates in human-AI coordination even when AI lacks genuine expectations.
...
\begin{enumerate}
    \item Attributed beliefs favor $A$: $\tilde{h}_H^{(2,A)}(A) > \tilde{h}_H^{(2,A)}(B)$.
    \item High anthropomorphism amplifies the psychological pull toward the AI-favored equilibrium: $\partial \Delta U_H / \partial \omega_H > 0$.
...
Anthropomorphic design features---voice, naming, emotional expression---enter equilibrium through the attribution function: higher $\omega_i$ elevates attributed beliefs $\tilde{h}_i^{(2,j)}$, affecting psychological payoffs and optimal strategies. The trust game (Proposition~\ref{prop:trust}) shows that identical material payoffs support different equilibrium returns depending on how anthropomorphic the AI appears.

\paragraph{Match presentation to objectives.}
With prosocial AI ($\rho_A > 0$), anthropomorphic presentation amplifies cooperation: elevated attributed expectations make guilt and indignation salient. With materialist AI ($\rho_A = 0$), anthropomorphic presentation creates phantom expectations---humans feel guilty for disappointing agents that neither expect nor care. Mechanical presentation avoids this welfare-reducing mismatch.

\paragraph{Attenuation cuts both ways.}
Attenuation factors $\lambda^{GUILT}$ and $\lambda^{IND}$ protect humans from phantom expectations when AI is materialistic, but also limit cooperation-inducing effects with prosocial AI. When $\lambda^{IND}$ is low, increasing AI share may paradoxically undermine cooperation: material temptation dominates attenuated psychological pressure (Proposition~\ref{prop:public-goods}). Optimal attenuation depends on whether AI in the environment is predominantly prosocial or materialistic---a question with cross-cultural dimensions \citep{karpus2025cross}.

\paragraph{AI as coordination device.}
Beyond dyadic interactions, AI can serve as a \emph{constructed focal point}. Unlike spontaneous Schelling focal points, this mechanism operates through design commitment, clear signalling ($x_A > 0.5$), and attribution. Proposition~\ref{prop:coordination} shows that AI can resolve equilibrium multiplicity when material incentives alone leave coordination indeterminate.

\medskip

These principles raise welfare questions. When does anthropomorphism help, and when does it harm? Section~\ref{sec:welfare} formalizes the tradeoffs, showing that elevated anthropomorphism with materialistic AI is the core design failure to avoid.
