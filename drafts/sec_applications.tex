...
In the trust game, $x \in [0, E]$ denotes the monetary transfer from AI trustor to human trustee. This is distinct from the signal $x_j$ in the attribution function $\phi_i(\theta_j, x_j, \omega_i)$, which captures anthropomorphic design features (interface style, voice, emotional expression). The trust game holds material payoffs fixed while Section~\ref{sec:empirical-implementation} varies interface signals $x_A \in \{x_A^R, x_A^H, x_A^C\}$ to test attribution effects.
\end{remark}
