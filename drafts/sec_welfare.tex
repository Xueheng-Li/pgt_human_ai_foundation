...
\begin{remark}[Strategy Spaces]
Propositions~\ref{prop:welfare-anthro}--\ref{prop:optimal-design-extended} apply to both discrete and continuous strategy spaces. The public goods application uses binary contributions ($c_i \in \{0, E\}$), yielding pure-strategy equilibria. The trust game uses continuous strategies ($x \in [0, E]$, $y \in [0, 3x]$), admitting interior solutions. The welfare results---that anthropomorphism effects depend on AI design---hold under both specifications.
\end{remark}
...
Proposition~\ref{prop:over-anthro} reveals a fundamental asymmetry. With prosocial AI, attributed expectations align directionally with genuine AI preferences, creating mutual gains through elevated cooperation. With materialist AI, any attributed expectation is phantom---existing only in the human's psychological model---and when phantom expectations exceed feasible returns, guilt imposes deadweight loss.

This asymmetry creates an alignment problem: prosocial AI with a mechanical interface may fail to elicit cooperation; materialist AI with an anthropomorphic interface harms welfare by inducing guilt without offsetting benefits.
...
\begin{proof}[Proof Sketch]
Condition (i): When attributed expectations are feasible, guilt vanishes and extended welfare reduces to material welfare. Condition (ii): With materialist AI, both objectives maximize at $x = 0$.
\end{proof}
...
\begin{proof}[Proof Sketch]
Condition (i): The extended welfare planner counts phantom guilt that the material planner ignores, yielding $x^{*,ext} < x^*$. Condition (ii): The extended welfare planner faces phantom guilt as an additional marginal cost, optimizing at a lower signal.
\end{proof}
...
Attenuation ($\lambda < 1$) reduces psychological costs from phantom expectations but also limits cooperation gains from prosocial AI. The optimal attenuation level depends on population composition: high attenuation protects against materialist AI but undermines prosocial AI's cooperation benefits.
