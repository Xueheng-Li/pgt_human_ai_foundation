...
\label{rem:RAE-vs-ZA}
RAE and the zero-anthropomorphism benchmark are distinct concepts:
\begin{itemize}
    \item \textbf{Zero-anthropomorphism} (Definition~\ref{def:zero-anthro}) characterizes attributed beliefs when $\omega_i = 0$. This is a limiting case of the attribution function, independent of equilibrium strategies.
    \item \textbf{RAE} (Definition~\ref{def:RAE}) requires attributed beliefs to equal equilibrium strategies. This is a fixed-point condition that may hold at any anthropomorphism level $\omega_i \in [0,1]$.
\end{itemize}
Neither concept implies the other. A zero-anthropomorphism benchmark $\tilde{h}_i^{(2,j),ZA} = 0$ typically differs from equilibrium strategy $\sigma_i^* > 0$, so zero-anthropomorphism does not yield RAE. Conversely, RAE can hold with positive anthropomorphism if the attribution function happens to project equilibrium play.
\end{remark}
